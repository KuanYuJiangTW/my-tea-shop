        ...
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="text-tea-text-muted hover:text-tea-green-ink transition-colors text-sm font-medium tracking-wide"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2">
                ...
                {user ? (
                  <>
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl hover:bg-tea-green-mist transition-colors text-sm text-tea-text-muted hover:text-tea-green-ink"
                    >
                    ...
                    {isUserMenuOpen && (
                      <div className="absolute right-0 top-full mt-2 w-44 bg-white rounded-2xl shadow-lg border border-[#EDE8DC] py-1.5 z-50">
                        <Link
                          ...
                          className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-tea-text hover:bg-tea-green-mist hover:text-tea-green-ink transition-colors"
                        >
            ...
            <Link href={lp("/cart")} className="relative p-2 group">
              ...
              {totalItems > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-tea-green-ink text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-medium">
                  {totalItems}
                </span>
              )}
            </Link>
          ...
          <div className="md:hidden py-4 border-t border-tea-green-pale space-y-1">
            {navLinks.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="block py-2.5 px-2 text-tea-text-muted hover:text-tea-green-ink hover:bg-tea-green-mist rounded-lg transition-colors"
            ...
            {/* AI 茶葉顧問 */}
            <button
              className="flex items-center gap-2 w-full py-2.5 px-2 text-tea-text-muted hover:text-tea-green-ink hover:bg-tea-green-mist rounded-lg transition-colors"
              onClick={() => {
                setIsMenuOpen(false);
