    ...
    <>
      ...
      {!isOpen && (() => {
        ...
        return (
          <button
            ...
            className={`fixed right-4 z-50 bg-tea-green-ink text-white rounded-full flex items-center transition-all duration-500 ease-in-out bottom-20 md:bottom-6 ${
              showLabel
                ? "px-4 py-2.5 md:px-5 md:py-3 gap-2 shadow-lg hover:bg-tea-green-deep hover:scale-105"
                : isIdle
                  ? "w-7 h-7 shadow-sm opacity-40 justify-center"
                  : "w-11 h-11 md:w-14 md:h-14 shadow-lg hover:bg-tea-green-deep hover:scale-105 justify-center"
            } ${
      ...
      {isOpen && (
        ...
        >
          ...
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-tea-green-ink text-white rounded-t-none md:rounded-t-2xl flex-shrink-0">
                ...
                <div className={`max-w-[80%] flex flex-col gap-0.5 ${msg.role === "user" ? "items-end" : "items-start"}`}>
                  <div
                    className={`px-3 py-2 md:px-3 md:py-2 rounded-2xl text-sm leading-relaxed ${
                      msg.role === "user"
                        ? "bg-tea-green-ink text-white rounded-tr-md"
          ...
          <div className="px-3 py-2 border-t border-tea-green-pale bg-white flex-shrink-0" style={{ paddingBottom: isMobile && keyboardHeight === 0 ? "calc(0.5rem + env(safe-area-inset-bottom))" : undefined }}>
            <div className="flex items-end gap-2">
              ...
              <button
                ...
                className="w-10 h-10 flex items-center justify-center rounded-full bg-tea-green-ink hover:bg-tea-green-deep disabled:opacity-40 text-white transition-all active:scale-90 flex-shrink-0"
                aria-label={t("send")}
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
