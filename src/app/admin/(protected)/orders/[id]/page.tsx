      ...
      <div className="flex flex-wrap items-start gap-4 mb-6">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <h1 className="text-xl font-bold text-tea-text font-serif">
              訂單 #{shortId(order.id)}
            </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-tea-cream-dark overflow-hidden">
            <div className="px-5 py-4 border-b border-tea-cream-dark">
              <h2 className="font-semibold text-tea-text text-sm">訂購品項</h2>
            </div>
            <div className="p-5">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-tea-cream">
                  {items.map((item, i) => (
                    <tr key={i}>
                      <td className="py-3 text-tea-text font-medium">{item.name}</td>
                      <td className="py-3 text-center text-tea-text-light">{item.quantity}</td>
                      <td className="py-3 text-right text-tea-text-light">NT${item.unitPrice?.toLocaleString()}</td>
                      <td className="py-3 text-right font-semibold text-tea-text">NT${item.subtotal?.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              {/* Totals */}
              <div className="mt-4 pt-4 border-t border-tea-cream-dark space-y-1.5">
                ...
                <div className="flex justify-between text-base font-bold text-tea-text pt-1 border-t border-tea-cream-dark">
                  <span>總計</span>
                  <span className="text-tea-green">NT${order.total_amount.toLocaleString()}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          {/* Customer */}
          <div className="bg-white rounded-2xl border border-tea-cream-dark p-5">
            <h2 className="font-semibold text-tea-text text-sm mb-3">顧客資訊</h2>
          ...
          {/* Shipping */}
          <div className="bg-white rounded-2xl border border-tea-cream-dark p-5">
            <h2 className="font-semibold text-tea-text text-sm mb-3">配送資訊</h2>
            <div className="space-y-2">
              <Row
                label="方式"
          ...
          {/* Payment */}
          <div className="bg-white rounded-2xl border border-tea-cream-dark p-5">
            <h2 className="font-semibold text-tea-text text-sm mb-3">付款資訊</h2>
            <div className="space-y-2">
              <Row
                label="方式"
