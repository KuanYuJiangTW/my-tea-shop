...
export function getMemberStatusCls(orderStatus: string, paymentStatus: string): { labelKey: string; cls: string } {
  ...
  const map: Record<string, { labelKey: string; cls: string }> = {
    ...
    shipped:   { labelKey: "orderStatus.shipped",   cls: "bg-tea-green-dark text-white" },
                  ...
                  <div className="flex items-start gap-2 bg-tea-green-mist/50 border border-tea-green-pale rounded-xl px-4 py-3">
                    ...
                    <div>
                      ...
                      <button
                        ...
                        className="mt-1.5 text-xs text-tea-green-ink hover:text-tea-green-dark underline"
                      >
          ...
          <div className="space-y-3">
            {bookingList.length === 0 ? (
              <div className="bg-white rounded-2xl border border-tea-green-pale p-12 text-center">
                ...
                <Link href={lp("/experiences")} className="text-sm text-tea-green-ink hover:text-tea-green-dark font-medium underline underline-offset-2">
                  {t("bookings.browse")}
                </Link>
              </div>
            ) : (
                  ...
                  <div className="mb-5">
                    ...
                    {tierHistory.length > 0 && (
                      <div className="mt-3 space-y-1">
                        <p className="text-xs font-medium text-tea-text-muted">{t("rewards.tierHistoryTitle")}</p>
                        {tierHistory.map(h => (
                  ...
                  {pointTransactions.map(tx => (
                    <div key={tx.id} className="flex justify-between items-center py-2 border-b border-tea-green-pale/60 last:border-0">
                      <div>
                        ...
                        <p className="text-xs text-tea-text-muted">{new Date(tx.created_at).toLocaleDateString(dateFmt)}</p>
                      </div>
                    ...
                    {isExpanded && (
                      <div className="border-t border-tea-green-pale px-6 py-4 bg-tea-cream-light/30 space-y-4">
                        ...
                        {order.reviewables.length > 0 && (
                          <div>
                            <p className="text-xs font-semibold text-tea-text-muted uppercase tracking-wider mb-2">
                              {t("orders.reviewTeas")}
                            </p>
                            <div className="flex flex-wrap gap-2">
                              {order.reviewables.map((r) =>
                                r.has_review ? (
                                  <span
                                    key={r.productId}
                                    className="text-xs text-tea-text-muted border border-tea-green-pale rounded-full px-3 py-1.5"
                                  >
                        ...
                        <div>
                          ...
                          {canEditAddress && (
                            <button
                              ...
                              className="mt-2 text-xs text-tea-green-ink hover:text-tea-green-dark font-medium underline underline-offset-2"
                            >
