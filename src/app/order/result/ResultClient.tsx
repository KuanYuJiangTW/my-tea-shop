    ...
    return (
      <div className="min-h-screen bg-tea-cream-light flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-tea-text-muted mb-2">{t("paypalCancelDesc")}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            {paypalOrderId && (
              <>
                <button onClick={handlePaypalRetry} disabled={paypalRetrying || paypalCancelling}
                  className="bg-tea-green-ink hover:bg-tea-green-deep disabled:opacity-60 text-white px-8 py-3.5 rounded-full font-medium transition-colors">
            ...
            <Link href={lp("/")}
              className="border border-tea-green text-tea-green-ink hover:bg-tea-green-ink hover:text-white px-8 py-3.5 rounded-full font-medium transition-colors">
              {t("backHome")}
            </Link>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-tea-cream-light flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-tea-text-muted mb-2">{t("paypalCaptureFailedDesc")}</p>
      ...
      <div className="text-center max-w-md">
        {isSuccess ? (
          <>
            ...
            {isBooking ? (
              <>
                ...
                <p className="text-tea-text-muted mb-2">{t("bookingSuccessDesc")}</p>
                {tradeNo && (
                  <p className="text-xs text-tea-text-muted mb-2">
                    {t("bookingRef")}<span className="font-mono font-medium">{tradeNo}</span>
                  </p>
                )}
                {user?.email ? (
                  <p className="text-tea-text-muted text-sm mb-4">{t("emailSentBooking", { email: user.email })}</p>
                ) : (
                ...
                <div className="bg-[#F0F6F1] rounded-2xl px-6 py-4 text-left mb-10">
                  ...
                  <ul className="space-y-1.5 text-sm text-tea-text-muted">
              ...
              <>
                ...
                <p className="text-tea-text-muted mb-2">{t("orderSuccessDesc")}</p>
                {tradeNo && (
                  <p className="text-xs text-tea-text-muted mb-2">
                ...
                {user?.email ? (
                  <p className="text-tea-text-muted text-sm mb-4">{t("emailSentOrder", { email: user.email })}</p>
                ) : (
                  <p className="text-sm text-amber-600 mb-4">
          ...
          <>
            ...
            <p className="text-tea-text-muted mb-10">{rtnMsg || t("paymentFailedDefault")}</p>
          </>
        )}
