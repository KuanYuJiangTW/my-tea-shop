...
const FALLBACK_CONTENT: Record<string, Omit<ExperienceContent, "slug" | "name" | "nameEn">> = {
  ...
  // 與上面那款並存：450 元看完就走，這一款是待著等鳥況最好的那兩小時。
  ...
  "egret-half-day": {
    ...
    includes:   [
      ...
      // 「炭火小點」要寫得出品名才有畫面。定案是梅山龍眼乾＋炭火烤麻糬：
      // 一個是在地名產、配茶對味、常溫放得住；一個烤起來會膨脹，有戲、小孩愛。
      // 兩樣都是常溫耐放的乾貨——山上不適合備生鮮，這是選品的第一條件
      "炭火小點每人一份（梅山龍眼乾＋炭火烤麻糬）",
      "萬鷺朝鳳生態解說手冊一份",
      "信淳茶居停車位（7 個車位，現場有洗手間）",
    ],
    notes:      [
      "集合地點：信淳茶居（本身就是停車場，7 個車位），車子可以開到門口，適合推嬰兒車與長輩",
      "現場有洗手間與遮蔭座位——四個小時的行程，這是帶長輩與小孩的人最需要知道的一件事",
      "觀景視野沒有電線橫過，長焦取景不必閃避",
      "本活動限定期間：8 月 22 日至 10 月 11 日（依鷺鳥族群實際抵達狀況可能微調）",
