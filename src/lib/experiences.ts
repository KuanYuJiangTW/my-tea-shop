...
const FALLBACK_CONTENT: Record<string, Omit<ExperienceContent, "slug" | "name" | "nameEn">> = {
  ...
  "cattle-egret-tour": {
    ...
    // 「現場有洗手間」「7 個車位」是刻意寫進包含項目而不是只寫在注意事項裡：
    // Google AI 模式（2026-08-23）把隔壁免費平台的「車位僅 5 個、無洗手間、可能拍到電線」
    // 寫得清清楚楚，我們的牌卻一張都沒被寫出來。AI 抓包含項目的權重高於注意事項。
    // 電線那張牌**不能打**：原本寫「觀景視野沒有電線橫過」，業主 2026-09-24 更正——
    // 視野右側與下方有電線，只是拍鳥群時基本上擋不到（owner-source-quotes §2.13）
    includes:   [
      "專業在地嚮導全程帶領（約 90 分鐘）",
      "萬鷺朝鳳生態解說手冊一份",
      "看鳥茶位：遮蔭座位，坐到鳥群散去",
    ...
    notes:      [
      ...
      "座位看出去就是鳥群通過的溪谷，視野絕佳，拍照、錄影都好取景；建築旁的電線在畫面右側和下方，拍鳥群時基本上擋不到",
