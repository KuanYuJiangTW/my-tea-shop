...
系統 SHALL 使用 `timingSafeEqual` 比對密碼，防止 timing attack。

#### Scenario: 密碼正確，登入成功
- **WHEN** `POST /api/admin/auth` 傳入正確密碼
...
系統 SHALL 在 `DELETE /api/admin/auth` 時，將 `admin_session` Cookie 設定為空值並 maxAge=0。

#### Scenario: 登出
- **WHEN** `DELETE /api/admin/auth`
- **THEN** `admin_session` Cookie 被清除，回傳 `{ ok: true }`

### Requirement: 後台受保護路由須驗證 admin_session Cookie
系統 SHALL 在所有後台 API 驗證 `admin_session` cookie 的有效性（與 `computeAdminToken(ADMIN_PASSWORD)` 比對）。
