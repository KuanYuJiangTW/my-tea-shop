...
`src/app/robots.ts` SHALL 輸出兩組規則：一般 `*` 群組與 AI 爬蟲群組（GPTBot、OAI-SearchBot、ChatGPT-User、ClaudeBot、Claude-User、Claude-SearchBot、PerplexityBot、Perplexity-User、Google-Extended、Applebot、Applebot-Extended、meta-externalagent、Meta-ExternalFetcher、Amazonbot、DuckAssistBot、MistralAI-User），兩組皆 `Allow: /` 且 Disallow 清單完全一致：`/cart`、`/checkout`、`/order/`、`/api/`、`/admin`、`/auth/`、`/account`。
...
- **THEN** 內容不得出現 `Content-Signal:` 行，亦不得出現任何 AI 爬蟲的全站 `Disallow: /` 條目；若出現，表示 Cloudflare「Managed robots.txt」被重新開啟，需回到 AI Crawl Control → Signals 關閉
