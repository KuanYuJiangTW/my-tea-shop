...
## 9b. 業主決策後的商品調整（**已撤銷，2026-08-23**）

> 這一章做完之後被推翻了。650 元的半日**不上架**：它沒有多賣任何東西——
> 核心內容（一壺可續水的茶、每人一份小點）在 450 元導覽上用現場加購就拿得到，
> 3 人時 1,800 元 vs 1,950 元，內容一樣還更便宜。等鳥茶席的概念活下來了，
> 但它是現場加購，不是可預約的商品。線上維持 0／150／450 三層。
> 推導見 `egret-pricing-and-onsite-copy.md`。
>
> 收尾：資料庫那筆留著當紀錄（`is_active = FALSE`）；上架指南已刪；
> `FALLBACK_CONTENT` 已移除；`add_egret_half_day.sql` 改寫成不上架的紀錄；
> 測試改成守「不得被誤上架」。

- [x] ~~9b.1 新增 `experience_types` 記錄「萬鷺朝鳳半日（含等鳥茶席）」~~ → 已建立但關閉，不上架
- [x] ~~9b.2 Sanity 建立該款的雙語內容~~ → 撤銷，上架指南已刪
- [x] ~~9b.3 `FALLBACK_CONTENT` 補該款備援~~ → 已移除
- [x] 9b.4 確認 250 元的單純導覽仍保留 → 實際定案為 **450 元／最低 2 人**（見 `adjust_experience_pricing.sql`，已執行）
