...
工序文案 SHALL 具名呈現實際使用的設備：甲種乾燥機（初乾）、擠壓機與布球揉捻（布球團揉）、箱型焙茶機（焙火）。SHALL NOT 以「專業設備」「先進機具」等空泛詞彙取代。
...
焙火 SHALL NOT 被呈現為所有茶款共通的必經步驟。各茶款狀態：四季春為 `skipped`（不焙，直接販售）、高山烏龍為 `optional`（部分焙火、部分以生茶販售）、蜜香紅茶為 `common`（回家後以箱型焙茶機調整風味）。

`optional` 的說明文字 SHALL 提供判斷依據（依該批茶的狀態決定焙或不焙、焙至何種程度），SHALL NOT 使用「有時候會焙」等模糊表述。

#### Scenario: 四季春不焙火

- **WHEN** 使用者選擇「四季春」並檢視共通後段
- **THEN** 「焙火」以降階樣式呈現並說明此款不焙火、回家後直接販售

#### Scenario: 高山烏龍焙火為選配

- **WHEN** 使用者選擇「阿里山高山烏龍茶」並檢視「焙火」
- **THEN** 該步標示為視批次決定，並說明不焙者以生茶販售，且提供焙與不焙的判斷依據
