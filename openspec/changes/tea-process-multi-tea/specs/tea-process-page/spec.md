...
切換茶款時，共通前後段的**步驟組成**（步驟集合與順序）SHALL 保持不變，結構性的變動 SHALL 僅發生在分歧段。

共通段的個別工序 MAY 帶有該茶專屬的強化說明（`accent`）或條件性狀態（`optional`／`skipped`）——例如蜜香紅茶的採摘需著蜒、四季春為機採、紅烏龍的室內萎凋與浪菁較重、四季春不焙火。此類差異 SHALL 以四態標記呈現，SHALL NOT 因「共通段」之名而被抹平成統一文案。

> **措辭沿革（2026-07-30 店主裁決）**：本條原文為「切換茶款時，SHALL 僅分歧段內容變動，共通前後段 SHALL 保持不變」。該措辭與本規格自身的「焙火為條件性工序」要求矛盾——`roast` 屬共通後段，卻必須在四季春為 `skipped`、高山烏龍為 `optional`、紅烏龍為 `accent`；也與經店主校對的 `design.md` 1.2 矩陣矛盾（`pick` 於蜜香紅茶標為強化）。經店主裁決改為「**步驟組成**不變」。
...
#### Scenario: 切換茶款時共通段的步驟組成不動

- **WHEN** 使用者從高山烏龍切換至蜜香紅茶
- **THEN** 共通前段仍為採摘、日光萎凋、室內萎凋、浪菁四步、共通後段仍為初乾、布球團揉、乾燥・揀枝、焙火、包裝五步，順序與步驟集合皆不變；結構性變動只出現在分歧段

#### Scenario: 共通段允許該茶專屬的強化說明

- **WHEN** 使用者切換至蜜香紅茶並檢視共通前段的「採摘」
- **THEN** 該卡片顯示著蜒（小綠葉蟬叮咬）的專屬說明並標記為強化，而非高山烏龍的共通採摘文案——步驟本身仍在原位、仍是第 01 步
...
#### Scenario: 跳過的工序保留卡片但降階
- **WHEN** 某款茶的某道工序狀態為 `skipped`（例如四季春不焙火）
- **THEN** 卡片仍然出現在序列中，標題劃線並以 `opacity-50` 降階，編號欄顯示破折號而非數字，
  卡片內容改印該茶的 `skipReason`
  ——保留而非移除，是因為「這款茶沒有這一步」本身就是要傳達的對比資訊

#### Scenario: 條件性工序標示判斷依據
- **WHEN** 某道工序狀態為 `optional`（此步視批次決定）
- **THEN** 卡片正常顯示，並加註此步視批次決定，同時說明判斷依據，
  不使用會讓讀者誤以為「一定有做」或「一定沒做」的措辭

#### Scenario: 編號依該茶自身順序，不用固定槽位
- **WHEN** 比對高山烏龍與紅烏龍的工序序列（前者炒菁在最前、後者在最後）
- **THEN** 兩款茶各自從 01 連續編號至 N，同一道工序在兩款茶可以是不同編號；
  `skipped` 的工序不佔用編號
