...
- [ ] 6.6.4 **FAIL 不成立，但暴露規格內部矛盾——待店主裁決**：checker 依 Scenario「切換茶款時共通段不動」（spec.md:103-106，「共通前段與共通後段的卡片內容不變」）判 FAIL，理由是蜜香紅茶的 `pick` 被標為 accent（著蜒）。

  但這條 Scenario 與**同一份規格的其他要求互相矛盾**：
  - 規格「焙火為條件性工序」要求 `roast`（屬**共通後段**）於四季春 skipped、高山烏龍 optional、紅烏龍 accent——即共通後段的內容本來就必須因茶而異
  - `design.md` 1.2 矩陣（經店主校對）明列 `pick` 於蜜香紅茶為「強化（著蜒）」、`witherIndoor`／`shake` 於紅烏龍為「強化」
  - 著蜒是店主確認的事實（tasks 0.4／0.5），刪掉它等於丟棄已確認的製程事實

  同 Requirement 下的另一條 Scenario「五款茶共用前段」要求共通前段四步「皆存在且**不被標為跳過**」——這條本實作是 PASS（accent 不是 skipped）。

  **研判**：「共通段不動」的合理解讀是「**步驟組成**不變，個別工序可有該茶專屬強化說明」，否則焙火要求無法成立。建議修規格措辭而非改程式碼。**未擅自修規格，也未刪除著蜒事實**——請店主裁決。
