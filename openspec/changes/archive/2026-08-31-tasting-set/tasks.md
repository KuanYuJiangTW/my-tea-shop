...
- [x] 6.4 端對端：下單一組 → 確認三款成分各扣 1 → 取消 → 確認各回補 1
      ✅ 端對端（資料庫層）：扣減前 烏龍50／紅茶53／金萱48 → `decrement_bundle_stock(1,1)` 後 49／52／47（**三款各扣 1**）→ 依成分快照 `increment_stock` 回補後回到 50／53／48（**完全還原**）。**API 層端對端由業主於 2026-08-15 完成**：兩筆真實 COD 組合訂單（a9f57cde completed、83f998b0 已取消），庫存由 50／53／48 → 兩次建單各扣 1 → 一次取消回補 1 → 實際落在 **49／52／47**，與「扣 2 補 1」完全吻合。訂單的 `bundleItems` 快照皆為完整 3 筆、單價 650（非成分加總 700）、運費 150（650 < 免運門檻）。**取消回補的路由層因此得到實資料驗證**
- [x] 6.5 英文版 `/en` 無中文殘留
