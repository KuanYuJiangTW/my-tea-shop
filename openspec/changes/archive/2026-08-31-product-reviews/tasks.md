...
- [x] 8.4 業主實測會員中心的留評流程（2026-08-15）
      證據不只業主回報：資料庫裡兩則評價都帶 `order_id` 與 `user_id`、`source = 'site'`
      （後台建檔的話這兩欄會是 NULL），代表**真的是走 `POST /api/product-reviews` 寫進去的**，
      已購驗證那三道對真實訂單放行成功。兩則（蜜香紅茶、阿里山金萱各 1 則五星）
      在正式站 `/products` 顯示中；各自只有 1 則，所以照規格不顯示平均星等
