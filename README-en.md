...
1. **E-Commerce** — Product browsing, shopping cart, ECPay payment gateway (credit card / ATM / convenience store), order tracking
...
- **API Rate Limiting**: IP rate limiter (20 req/min), 5 failed login attempts per 15 min
...
│   ├── layout.tsx                        # Root layout (GA, Auth, Cart Provider)
│   ├── page.tsx                          # Home page
│   ├── about/                            # Brand story
...
Fill checkout form (shipping info + payment + delivery method)
     ...
     │
     └── Online Payment ──► POST /api/ecpay/checkout ──► Write to DB (pending)
                              │
                              ▼
  ...
  (verify signature → update to paid    (redirect to /order/result)
