...
| 現況寫法 | 對比 | 應改為 |
|---|---|---|
| `bg-tea-green` + `text-white`（含首頁主 CTA） | **3.05** | `bg-tea-green-ink` |
| `text-tea-green` 在米白上（「查看全部」等連結） | **2.85** | `text-tea-green-ink` |
| `text-tea-text-light` 內文（全站大量） | **3.43** | `text-tea-text-muted` |

> 這三項散在 39 個檔案，屬獨立的遷移波次。新寫的介面**不得**再產生這三種組合。
