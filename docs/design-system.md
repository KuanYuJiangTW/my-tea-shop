...
（`text-tea-green`）而不是工序色**，與 2.3 節那幾項同源。若日後要整組重做，
方向是「降彩度但不改色相」——溫度感留著，同時多數能順帶過門檻。
...
| `text-tea-green` 用在**價格／金額**（約 10 處） | **3.05** 白卡／**2.69** 米底 | `text-tea-green-ink`（5.15／4.54） |

**價格那一列是 2026-08-08 單獨拍板的**：小江看過 `tea-green` 與 `green-ink` 的並排
對照後決定維持原色。同一天也結案了「綠色降密度」——查證發現 733 處綠色裡絕大多數
有語意（hover 態、選中態、圖示、eyebrow、連結），真正可議的靜態綠字只有約 31 處。
