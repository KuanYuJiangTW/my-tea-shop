...
2. **Email 色值集中成具名常數**（`src/lib/email.ts`，503 處 hex）。色值已收斂完畢
   （第十三波），現在只剩結構問題：hex 散在 1550 行裡，改色要全檔搜尋。
   建一組常數（`const C = { text: "#3D4A42", muted: "#637169", ... }`）後改用 `${C.x}`。
   **零視覺變化**，驗證方式同第十三波：色值多重集合比對，總數應維持 503。
   施工前先確認每處 hex 都在模板字串（backtick）內——若有落在單引號字串裡的要另外處理
