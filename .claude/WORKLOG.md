...
2. **綠色降密度 ← 前提已被推翻，剩下的都要拍板**（查證見第十五波）。
   733 處裡有語意的佔絕大多數，真正可議的只有約 31 處靜態綠字，且都是風格選擇。
   兩個可拍板的切入點：
   - **價格改用 `tea-green-ink`**（約 10 處）：唯一有客觀依據的一項——
     `text-tea-green` 對比 2.69–3.05，價格是交易資訊，依原則 2 該用 ≥4.54 的 green-ink。
     代價是商品卡與結帳頁的視覺重心改變
   - **編號／項目符號改中性**（約 15 處）：`return-policy` 的圓圈數字、`ResultClient` 的
     ①②③、`page.tsx` 的哲學編號。純風格，沒有對錯
3. **釐清 `tea-green-pale` 與 `tea-cream-dark` 的分工**（design-system 2.1 節）：
   兩者現在都寫「邊框、分隔」，是隨手用綠的根源。但實測 `cream-dark` 在米底上
   對比只有 1.08（等於看不見），所以不能簡單地「中性邊框一律 cream-dark」——
   要嘛承認 `green-pale` 就是預設邊框色、要嘛新增一個夠深的中性邊框色
