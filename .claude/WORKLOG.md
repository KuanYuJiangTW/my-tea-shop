...
2. **Email 色值收斂 ← 卡在拍板**（`src/lib/email.ts`）。規範化部分已完成（`4e2b1cf`）：
   大小寫、3 碼展開、三個 ΔEok<0.011 的近似色已收，503 處 hex／相異色 33→30。
   **注意：Email 不能用 Tailwind class，必須維持內聯 hex**，所以這項是「收斂色值」
   不是「改用 token」。剩下 20 種色盤外色值分三組，都會真的改變信件外觀，需小江拍板：
   - **A 組（低爭議）**：米色系 7 種寫法 22 處（`#E8E2D8`10／`#E8E0D4`5／`#E8E0D2`2／
     `#FEF2F2`2／`#FEF9EC`2／`#FFF7ED`1／`#FFF8ED`1），ΔE 0.017–0.030。
     `#E8E0D2` vs `#E8E0D4` 只差 2 是手誤鐵證。建議全收進 `tea-cream-dark`
   - **B 組（最大宗）**：`#6B7B6E` **118 處**，是 email 的次要內文色。彩度 0.0274
     （莫蘭迪區間），但色盤裡沒有對應色——`tea-text-light #6B8872` 彩度較高、偏綠。
     三個選項：保留原色並正式納入色盤／改用 `tea-text-light`／改用 `tea-green-dark`(ΔE 0.024)
   - **C 組（差異最大）**：狀態色 10 種 27 處。email 用的是高彩度 Tailwind 紅／琥珀
     （`#DC2626`、`#D97706` 等，彩度 0.15–0.22），而前台已改用低彩度的
     `status-danger #7A4545`／`status-warn #92400E`（ΔE 0.07–0.19）。
     收斂會讓警示色明顯變柔和——**這是「信件警示要多強烈」的商業判斷，不是設計細節**
   - 拍板後才做「集中成具名常數」（先建結構後調值，屆時改色只需動一處）
3. 綠色降密度（`tea-green` 系列 621 處 vs `tea-cream` 現已 130+ 處，綠色仍被當成預設色用）——
