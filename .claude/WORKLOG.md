...
- Sanity **草稿**（未發布）：攻略文標題改「2026 萬鷺朝鳳最佳時間與地點｜…」、新增「黃頭鷺是什麼鳥？」「一天怎麼排？要不要過夜？」兩段、去掉「歸巢」、無人機改寫成縣府禁航公告；體驗頁 seoDescription／介紹同步去「歸巢／中繼棲地／roost」、「一杯」→「一罐」冷泡茶。變更全文見 commit 內本節與 owner-source-quotes

### 還沒做的

- **導覽頁搜尋摘要改寫待業主同意**：GSC 匯出顯示 `/experiences/cattle-egret-tour` 9,484 曝光、平均排第 2.6，點閱率只有 0.7%（攻略文 7.5%）。摘要現在讀起來像 450 元廣告；提案改成先回答「幾點最壯觀」再帶導覽（文案在 2026-09-23 對話）。
...
- **攻略文「太興村還有哪些地方可以看？」**：已給業主建議名單（阿村師的家、草本傳奇、太興翔億樓、鴻朋觀景咖啡；不列鷺露茶居＝名字跟信淳茶居太像；另補一句區分雲林的樟湖觀景台），等業主確認名單與「太興飛瀑停車場觀鷺平台」是什麼。
- **問業主：大眾運輸／接駁車／騎機車能不能上來**——GSC 出現 AI 模式的追問（「有接駁車」「騎機車」「大眾運輸」「高雄出發」），攻略文沒答。
