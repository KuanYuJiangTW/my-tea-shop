...
## 2026-08-31（續）PR #17 已合併上線＋三處 Sanity 文案修正

`main` 進到 `8ccec91`。Vercel 已部署，正式站逐項驗過：影片、四張圖、10 個小標錨點、
目錄、季節倒數、浮動 CTA、同日交叉銷售、10 個 `tel:` 連結全部到位；公告條未掛載
...
### 用 write token 改的三處（業主逐次授權，寫入即上線）

1. 付款：「現場付現」→「現場付現或轉帳（不接受刷卡）」
2. 摘要：把兩個地點的條件分開講（原文把「停車不用錢」和「有洗手間與遮蔭座位」
   寫在一起，會讓人以為免費平台也有廁所椅子）
3. 「這件事是怎麼開始的」：推廣功勞改回「景觀平台停車場＋附近幾戶鄰居的觀景平台
   一起推廣，不是哪一家的功勞」
4. `updatedAt` → `2026-08-31T04:00:00.000Z`（挑 04:00Z 讓 UTC 與台北都落在 8/31，
   避開跨日邊界；頁面是伺服器端格式化，Vercel 的 TZ 是 UTC）

每次寫入前都先比對原文一字不差、確認小標索引正確，並用 `ifRevisionID` 鎖版本。

### 業主確認過的事實（我錯過兩次，已寫進 memory）

**兩個賞鳥地點都是他家的**：景觀平台停車場（免費、有車位、**無座位無洗手間**）、
信淳茶居（150 元／人、含停車、有洗手間與遮蔭座位）。
但**照片影片裡出現的建築物一律不是自家的**——所有鳥的影片都是從茶居往外拍的。
細節見 memory 的 `project_egret_venue_facts`。

### 還沒做的

- 浮動條「捲到 35% 滑入」與「ChatWidget 被頂上去」**只用對照組證明機制正確**，
  預覽窗格因 `document.hidden` 驗不到。已上線，請在真手機確認一次。
  時機不對就調 `FloatingGuideCta` 的 `showAfter`。
- 三處文案修正要等 ISR（`revalidate: 3600`）到期才會顯示，不需重新部署。
