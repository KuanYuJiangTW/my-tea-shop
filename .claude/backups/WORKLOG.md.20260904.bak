...
- **PR #28 尚未合併**。
- **第四階「照片」完全沒動**：照片覆蓋率 36.9% → 目標 55–65%（同類品牌 50–85%）。
  商品照要改 4:5 直式、敘事段落補實景照。**瓶頸是攝影不是程式**，需要業主出素材。
  這一階才是「更有質感」影響最大的一項。
- 商品詳情頁仍不存在（所有商品擠在 `/products` 一頁，高單價茶葉沒有各自的著陸頁）。
- `cta-tea-soft`／`cta-visit-soft` 兩色已定義但**目前無人使用**（季節條帶試過後被否決）。
  若一直沒用途，下次整理色盤應刪除，不要留孤兒 token。
- `tracking-eyebrow` 已定義未套用；eyebrow 現況散寫 `[0.25em]`／`[0.3em]`。
- **`.claude/WORKLOG.md` 已達 11 節，超過 MAINT-4 的 10 節上限**，需要壓縮。
  但最舊兩節裡，「紅茶製作 3 小時場」那節是**業主對製程的逐字原話**（文案的依據來源），
  我沒有自行壓掉——要壓請先決定那段原話搬去哪（建議搬進體驗內容文件而非刪除）。
