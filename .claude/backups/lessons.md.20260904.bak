...
## 2026-09-02 dev server 開著跑 vitest，兩次換來一模一樣的 57 個假紅

- 情境：同一天發生兩次，都是**改完檔案後 dev server 正在重編譯**時跑 `npx vitest run`。第一次是併行送出 vitest 與 tsc，第二次只是用 `&&` 串在同一則指令裡（已經是依序了），照樣中招——**所以肇因不是併行，是 dev server 在寫 `.next`**。
- 代價：兩次都報 **57 failed / 920 passed**，而且總數都是 977（正常 1142）。第一次差點被我當成改壞了去回溯 debug。
- 判準：**先看總測試數對不對**。少於平常（本專案 1142）就是跑到一半中斷，不是斷言失敗——直接重跑，不要開始 debug。同一則裡 `npm run lint` 若噴 `enoent`，是 cwd 被帶走，`cd` 到專案根目錄再跑。
- 規則：**跑四件套前先確認 dev server 不在重編譯**（剛改完檔就等幾秒），或乾脆停掉 dev server 再跑。四件套本身也一律依序、不要併行。
