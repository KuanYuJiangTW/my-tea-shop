...
| 寫測試、補回歸測試、修紅掉的測試 | `tester`（本 repo 自訂，`.claude/agents/tester.md`） | 補測試、或修完 bug 要釘住它 |
| 對外宣稱一致性盤點 | `copy-guardian`（本 repo 自訂，`.claude/agents/copy-guardian.md`） | 動到價格、設施、包含項目、地點差異、或任何對外文字檔 |
| 實作前的方案設計 | `Plan`（內建） | 跨 3+ 模組或動高風險區前 |

**這四個自訂 agent（checker／judge／tester／copy-guardian）都開了 `memory: project`**（記憶在 `.claude/agent-memory/<name>/`，隨 repo 走）。
它們會跨 session 累積這個專案的經驗，所以**同類工作固定派同一個角色**——每次改派等於把記憶丟掉。
...
| 對外文案（價格、設施、包含項目、`llms.txt`、JSON-LD、雙語字串） | 交叉比對所有文案來源——**四件套全綠也證明不了文案對** | `copy-guardian` |
