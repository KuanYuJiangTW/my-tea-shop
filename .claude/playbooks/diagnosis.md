...
| **本機（Windows）：`python`／`python3` 是 Microsoft Store 的 app-execution alias 假殼** | `command -v python` **找得到**（`.../WindowsApps/python`），執行卻靜默結束、什麼都不做、**exit code 仍是 0**。2026-09-04 用 `python - <<'EOF'` 改 `lessons.md` 連中兩次，檔案完全沒動而指令看起來成功。**這台機器上不要用 python 腳本改檔**——用 Edit 工具，或 `node`（node 是真的）。與「`sed` 含 `#` 靜默失敗」同類：**改完一律回頭 grep 複驗，不要相信 exit 0** |
