...
| **本機（Windows）：`python`／`python3` 是 Microsoft Store 的 app-execution alias 假殼** | `command -v python` **找得到**（`.../WindowsApps/python`），執行卻靜默結束、什麼都不做。**exit code 不可靠**（2026-08 觀測到 49，2026-09-04 觀測到 **0**——別拿它當判準）。2026-09-04 用 `python - <<'EOF'` 改 `lessons.md` 連中兩次，檔案完全沒動而指令看起來成功。**這台機器上不要用 python 腳本改檔**——用 Edit 工具，或 `node`（node 是真的）。與「`sed` 含 `#` 靜默失敗」同類：**改完一律回頭 grep 複驗，不要相信 exit 0** |
| **`sed -i` 在本專案有兩種靜默失敗** | (1) 替換字串含 hex `#` 會與分隔符相撞，**不報錯也不替換**（2026-09-02，`661e480` 補救）；(2) 它**重寫整檔**，會把 CRLF 全部換成 LF（2026-08-31 一次換掉 910 行，之後 Node 的 `split("\r\n")` 才丟 TypeError）。**Windows 專案上不要用 `sed -i` 改 CRLF 檔案**，用 Edit 工具或 Node 讀寫並明確 `join("\r\n")`。非用不可時：分隔符改 `\|`，並在 `sed -i` 之後重跑 `file <檔案>` 才可以宣告行尾——**「我驗過了」要連同「在哪個動作之前驗的」一起記** |
...
| **Bash tool 的 heredoc 會吃掉一層反斜線；`node -e` 帶巢狀引號也會被 shell 吃掉** | 兩者都讓改檔腳本變成語法錯或靜默 no-op（2026-08-18 與 2026-08-25 各中一次；後者是 `/[^}]*/` 被吃成 `/?/`）。做法：**要寫進檔案的多行內容一律用 Write／Edit 工具，不要用 heredoc**（heredoc 只留給 commit 訊息這類一次性短文字；2026-08-31 內容裡的單引號讓外層引號配對失敗，整段被當成命令）。非用腳本不可時寫進 scratchpad 的 `.js` 再 `node <檔案>`，而且 **JS 內容盡量不要出現反斜線**——要 CRLF 就用 `String.fromCharCode(13,10)`，要正規表示式就直接寫進目標檔而不是經由產生器 |
...
| **同一個 hidden 狀態還會關掉四件事**（2026-08-30／08-31 各被騙一次，共花掉大半個 session）：影片自動播放、**IntersectionObserver**、**`scrollTo()` 觸發的 scroll 事件**、**既有元素的樣式重算** | 症狀長得完全像 bug：浮動條捲到 70% 都不出現、`calc(5rem + var(--x))` 把變數設成 200px 也不動。**兩個判別法**：(a) 手動 `dispatchEvent(new Event('scroll'))`，會動就不是 bug；(b) 建**對照組**——**全新建立**的元素套同一條 calc 會正確算出 280px，既有元素停在 80px，那就是重算被停掉而不是 CSS 寫錯。**「沒反應」不等於「壞了」**，要下結論請在真實瀏覽器開 |
