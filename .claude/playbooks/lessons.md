...
## 2026-09-24 兩段路寫在同一句，讀者把旁邊的「3 到 5 分鐘」套到了另一段

- 情境：把業主的交通原話寫進導航區塊：「搭公車到橫山站下車，再走到景觀平台停車場，信淳茶居就在旁邊（走路 3 到 5 分鐘）」。
- 代價：業主一眼看出讀起來像「下車走 3 到 5 分鐘就到」，實際橫山站到停車場要 20~24 分鐘。業主原話沒給這段時間，我沒寫，但把另一段的數字擺在它旁邊，等於替它編了一個時間。幸好還在 PR 裡沒上線；搭公車的客人若照舊版估回程時間，會趕不上下山的車。
- 規則：文案寫多段路程時，**每一段都寫出「從哪到哪、多久」**；某段時間未知，就明寫「這段要走一段路，時間請先問我們」，不要讓相鄰的數字被讀者套用過去。
- 去處：暫存於此（與上一條同屬 JUDG-11「對外宣稱要有依據」：這次沒寫錯字面，錯在讀者的合理解讀）。

## 2026-09-24 「視野沒有電線」沒有業主原話，還被我當賣點擴散到 7 處

- 情境：做萬鷺朝鳳 SEO 時，我把既有文案裡的「視野沒有電線橫過」當成已確認的賣點，又寫進攻略文「在哪裡看？」與 llms.txt。copy-guardian 查依據時也只查到「既有文案本來就有」就放行。
- 代價：業主更正——右側與下方有電線，只是拍鳥時擋不到。錯誤說法從 2026-08 上線，被我從 4 處擴散到 7 處，其中 3 處在 Sanity（直接上線）。帶長焦的客人一到現場就看得到。
- 規則：**「既有文案有寫」不等於「有依據」**。要把一句既有宣稱搬到新位置（尤其當賣點、寫進 llms.txt）之前，先在 `docs/owner-source-quotes.md` 找到它的業主原話；找不到就先問，不要沿用。
- 去處：暫存於此。這是一週內第三例（歸巢、兩段路程、電線），同屬 JUDG-11 (b)——**下次健檢應把「沿用既有宣稱也要查 owner-source-quotes」併入 JUDG-11，並寫進 copy-guardian 的檢查清單**。
