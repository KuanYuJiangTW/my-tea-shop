...
## 2026-08-26 從照片畫面推斷製程工序，把日光萎凋判成室內萎凋
- 情境：業主問首頁 hero 輪播該放哪幾張照片，附了 4 張日光萎凋現場照。我看到棚頂、採光罩、吊扇，判定其中兩張「不是日光萎凋，是室內萎凋／浪菁」，還下了「判準就一條：有沒有在屋頂下」，並據此建議改檔名、改放 `/process` 的哪一段
- 代價：業主當場糾正——**那是透光浪板，蓋來遮雨，日照不減**。整段分類全錯，連帶檔名與版位建議一起錯。而我在同一則回覆裡，正拿 WORKLOG 的 tea-making 前例警告業主「文案與事實不符不要犯第二次」。這次是當面問所以立刻被抓到；若是無人審閱的批次作業，這個分類會流進 alt／圖說／`/process`／`llms.txt`，變成線上的錯誤製程宣稱，而 AI 會抓去回答
- 規則：**照片只能用來判斷「畫面上有什麼」**（構圖、明暗、彩度、人物、雜訊、裁切後長怎樣——這些肉眼與程式都可驗），**不能用來判斷「這是哪道工序」**。要把工序名寫進檔名、`alt`、圖說、`/process` 段落或 `llms.txt` 之前，一律先問業主那張拍的是什麼；他回答之前用泛稱（repo 既有的 `做茶實景`／`Crafting tea by hand` 就是正確做法）或留空，不要填具體工序名。已確認的事實兩條：**霧抉茶的日光萎凋場蓋透光浪板遮雨，有屋頂 ≠ 不是日光萎凋**；把茶菁抛開的動作是**日光萎凋期間的翻動／攤晾**，不是浪菁
- 去處：暫存於此（與 JUDG-11 同源——對外的事實宣稱不得由推論產生。本條把範圍從「文案」延伸到「照片的工序標註」；單次發生，未升格）
