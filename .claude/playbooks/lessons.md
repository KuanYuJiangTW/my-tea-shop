...
## 2026-09-02 批次改色「一律改深」，把深色區塊上的小標從 3.05 打到 1.81

- 情境：重做 AA 對比遷移，用規則式替換把全前台 `text-tea-green` → `text-tea-green-ink`、`text-tea-text-light` → `text-tea-text-muted`。規則本身正確，但它看不見背景。
- 代價：深墨區塊（`bg-tea-text` #3D4A42）上的英文小標原本 `#7D9B84` 是 **3.05**（剛好過 UI 元件門檻），改深成 `#58745F` 後掉到 **1.81**——比改之前更看不見。8 處，橫跨首頁、關於我們、隱私權、退換貨四頁。grep 完全看不出來，四件套也全綠（顏色不影響型別與測試）。
- 規則：**批次替換顏色前，先確認受影響檔案有沒有深色區塊**（`grep -l 'bg-tea-text\|bg-<深色>'`）；有的話替換完**必須**用 `docs/contrast-audit.js` 實跑一次，並專門篩出「前景暗且背景暗」的節點。深色底上的文字要往**淺**的方向走（本專案用 `text-tea-green-pale`，在 #3D4A42 上 6.52），不是往深。
- 去處：已併入 `docs/design-system.md` 2.3 節的教訓框。與 JUDG-8「監控本身也要能變紅」同源——四件套對顏色是盲的，不能拿它當顏色改動的驗收。
...
## 2026-09-02 併行跑 vitest 與 tsc，換來 57 個假紅

- 情境：驗證四件套時把 `npx vitest run` 與 `npx tsc --noEmit && npm run lint` 放在同一則訊息裡併行送出，而 dev server 正開著、持續寫 `.next/dev`。
- 代價：vitest 報 **57 failed / 920 passed**，而且總數只有 977（正常是 1142）——跑到一半就中斷。差點被我當成改壞了去回溯 debug。依序重跑就是 1142 全過。同一則訊息裡 `npm run lint` 還噴 `enoent`，因為 shell cwd 被前一個併行指令帶走。
- 規則：**四件套一律依序跑，不要併行**；dev server 開著時更不要。判斷測試是不是真紅，先看「總測試數對不對」——數量少於平常就是跑到一半中斷，不是斷言失敗。
- 去處：暫存於此。與 JUDG-5「四件套不可省 tsc」同一節，補的是「怎麼跑」而不是「跑哪些」。
